...
internal static class EzTranspiler {
    ...
    internal static CodeMatcher Replace(this CodeMatcher match, CodeMatch[] pattern, CodeInstructions replacement,
        ...
        foreach (var instruction in PatchProcessor.GetCurrentInstructions(method.Method))
        {
            ...
            if (instructions.Count > 0)
            {
                ...
                if (lastInstruction.OpCode.LoadsLocalVariable())
                {
                    ...
                    [
                        locals[lastInstruction.GetIndex()].LocalType ?? throw new InvalidOperationException()
                    ]);
